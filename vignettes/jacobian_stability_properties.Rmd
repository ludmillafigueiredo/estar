...
This information is classically summarized in the *species interaction matrix* [also known as $B$ matrix, @novak_characterizing_2016, see Glossary in the main text].
In community models, this is equivalent to a time-discrete version [@downing_temporal_2020] of the Jacobian matrix composed of the partial derivatives of the dynamical equations describing each species population growth [@dominguez-garcia_unveiling_2019]. The Jacobian matrix consists of $m \times m$ entries (for $m$ species). Each entry reflects the effect of the population density of the species in the column on the per-capita growth rate of the species in the row. (Fig. 2)

![Figure 2: The $B$ matrix that the Jacobian functions in `estar` take in as argument is the $m \times m$ Jacobian matrix (for $m$ species). Each entry reflects the effect of the population density of the species in the column on the per-capita growth rate of the species in the row](figures/B_matrix.png){width=40%}

If time-series data on composition of real-world communities is gathered, then it can be used to estimate interaction strengths in those communities with multivariate autoregressive state-space (MARSS) models. To assist `estar` users, in this vignette, we focus on explaining [how a MARSS model works](#intro-marss-model) and how these models are used with the functions provided by the [`estar` package](#marss-in-estar).
To facilitate understanding, we illustrate the definition of a MARSS model in the context of an ecotoxicological study that applied insecticide to an aquatic community.
...
```{r echo = FALSE, message = FALSE,  fig.height = 6, fig.width = 8, fig.cap = " Figure 3: Organism mean count (log axis) of the five functional groups over the course of ecotoxicological experiment. The vertical dashed line identifies the time when chlorpyfiros insecticide was applied (at week 0), and facets represent the different concentrations (micro g/L)."}
...
-   from $\mathbf{X}$ and $\mathbf{Y}$ alone, it is impossible to trace how observations refer to processes. We will address this in the description of the observation process $\mathbf{Y}$.
...
Jacobian properties. The estimated $\beta_{(i,j)}$ reflect the effect of the abundances of species in the column (species j) on the per-capita growth rate of the species in the row (species i).
-   $\mathbf{u}$ is a $m \times 1$ vector of states fixed values. These are the intrinsic rates of increase of abundance for each species/functional group.
...
-   $\mathbf{w}_t$ is a $m \times 1$ vector of the state ($X$) process errors, which reflects effects of environmental stochasticity on the system.
...
The $w_{t}$ errors follow a multivariate normal distribution with mean 0, and covariance $\mathbf{Q}$ (a $m \times m$ matrix).
\begin{equation}
...
-   $\mathbf{y}_t$ is a $n \times 1$ vector with the number of $n$ observations taken at time $t$.
...
-   $\mathbf{D}$ is a $n \times q$ matrix of $\delta$ coefficients reflecting the effect of each of the $q$ covariates on each of the $n$ observations
...
When fitting MARSS models, the user can have up to eight matrices or vectors of parameters estimated (Holmes, Scheuerell, et al., 2024). The species interaction matrix ($\mathbf{B}$) is usually fully estimated, i.e. all interaction strengths are estimated. It is, however, possible to constrain values to specific values (e.g. to 0 when one species is known to not affect the other one) or in relation to each other. We showcase the use of MARSS in a system assumed to be at equilibrium and with zero observational error. These assumptions reflect knowledge about the system. We leave it for the user to define their own assumptions for their systems as these assumptions will affect the estimation of $\mathbf{B}$. Moreover, it is important to point out that the calculation of asymptotic resilience (Downing et al., 2020) implemented in the package requires that process error is zero and covariates are constant.
...
For simplicity we present a minimal case of one single observation per state process. However, it can be extended to systems with multiple observations per state process (by appropriate specification of matrix $\mathbf{Z}$).
...
-   Each observation corresponds to one state process ($n = m$), thus $\mathbf{Z}$ = "identity". In our case we have 5 functional groups, therefore $\mathbf{Z}$ corresponds to a $5 \times 5$ identity matrix:
...
-   All functional groups have the same state processes variance: $w_t = \sigma$ and dynamics of each functional group is not covarying with any other groups due to environmental drivers, thus `Q = "diagonal and equal"` (in MARSS terminology), which equates to \begin{equation}
    \begin{bmatrix}
          \sigma & 0 & 0 & 0 & 0 \\
          0 & \sigma & 0 & 0 & 0 \\
          0 & 0 & \sigma & 0 & 0 \\
          ...
          \end{bmatrix}
    \end{equation}\
-   Observational error is the same for all functional groups and is assumed to be 0 (we assume complete knowledge of the system): $v_t = 0$, thus `R = 0`
-   No covariates are assumed to affect system dynamics except for insecticide treatment (effect of which we expect to see in matrix $\mathbf{Z}$). Thus, $c_{t} = c = 0$
...
```{=tex}
\begin{equation}
X = \mathbf{B} \times X_{t-1} + 0 + C.0 + w \\

Y =  Z  \times X_t + 0 + D.0 + 0 \\
\end{equation}
```

Thus, our aquatic community data must be split by the concentration of the pesticide, into matrices $Y_{control}, Y_{0.1}, Y_{0.9}, Y_6, Y_{44}$, each containing the time-series of functional group counts under one treatment.
In this experiment we have two replicates per treatment and four replicates for the control but, for simplicity for this demonstration we are modelling one observation as corresponding to one state processes, we therefore use the mean of functional group abundances over all replicates. Another possibility would be to randomly pick one of the replicates per treatment.

Importantly, the data have to be supplied using a specific chosen time step (e.g. every week, month, year). Missing values for some functional groups at some time steps in observations are allowed (since this is a state-space model, those values will be estimated). In our experiment, during the first month, the community was sampled first every week and then bi-weekly; but after that the sampling was done every four weeks. We therefore have chosen one month (four weeks) as a time step for our analyses - to avoid the need of interpolation. In preparation of the data, we subsample the time steps accordingly.
...
## convert into time-series matrix per insecticide concentration
...
We provide the function `estar::extractB()` in `estar` which extracts $\mathbf{B}$ in a matrix form from the fitted MARSS object and names the interacting species/groups
...
A positive value indicates the system is reactive and the higher its value, the faster the perturbation can grow. The aquatic community in our example seems to be reactive even when no insecticide is applied. And, it is more reactive in treatments with higher insecticide concentrations.
...
The largest response of the community to the perturbation, it is a measure of the transient behavior of the system. The more the value of maximal amplification deviates from 1 (larger than 1), the more the system departures from its equilibrium. If maximal amplification %~~% 1, then the system is rather stable. In our case maximal amplifications increase for higher insecticide concentrations and are higher for 6 micro g/L compared to 44 micro g/L.
