...
IDEA TODO: this block is probably going to have to be a form of block that I call from most functions and then do specific stuff with, depending on the 

```{r}
(comp_table <- aquacomm_fgps |>
...
TODO: filter the time steps I want to demonstrate each case and split those for the matrix (not even sure that will be necessary).

```{r}
# this is a list of dissimilarity matrices
(comp_list <- comp_table %>% 
   dplyr::filter(time %in% seq(1, max(aquacomm_fgps$time))) %>% 
   group_by(time) %>% 
   group_split() %>%
   # each matrix contains the dissimilarities between the control and each treatment, for a given time step
   map(vegdist) %>% 
   map(., ~ as.matrix(.x)[,1]) %>%
   map(., ~ data.frame(dissim = .x[-1])))
```
```{r}
comp_list[[1]]
```
...
Sensu Hillebrand: initial, first time after disturbance
```{r}

```

We also offer at a time of choice
```{r}
...
To calculate the compositional stability, we need a matrix of species abundances where the rows are the disturbed and undisturbed states of the community. For 
