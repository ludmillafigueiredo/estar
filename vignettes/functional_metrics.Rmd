---
title: "Functional stability metrics"
output: 
...
(`response = "v"`) by time (over a time frame (`metric_tf = c(1, 28)`, Fig.
5-b).
...
comp_list <- comp_table[which(comp_table$time %in% seq(1, max(aquacomm_fgps$time))),] |>
  split(f = ~time) |>
  # each matrix contains the dissimilarities between the control and each treatment, for a given time step
  lapply(\(.) .[,-which(names(.) %in% c("treat", "time"))]) |>
  lapply(vegan::vegdist) |>
  lapply(\(.) as.matrix(.)[,1]) |>
  lapply(\(.) data.frame(dissim = .[-1]))
```
