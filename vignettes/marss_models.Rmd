...
```{r fig.cap = "Organism mean count (log axis) of the five functional groups over the course of ecotoxicological experiment. The vertical dashed line identifies the time when chloryfiros insecticide was applied (at week 0), and facets represent the different concentrations (micro g/L)."}
...
We have replicates but we are modelling one observation as corresponding to one state processes.
...
What are the bounds of the metric (min and max) and how to interpret it (e.g. the larger asymptotic resilience, the more stable is the system as such communities have a faster long-term return to stable state).
