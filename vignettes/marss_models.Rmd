...
These measures are classically summarized in the *community matrix* [sensu @novak_characterizing_2016, detailed in the Glossary in the main text].
...
```{r fig.cap = "Organism mean count (log axis) of the five functional groups over the course of ecotoxicological experiment. The vertical dashed line identifies the time when chloryfiros insecticide was applied, represent the different concentrations (micro g/L)."}
...
-   $\mathbf{B}$ is a $m \times m$ matrix of states covariances. This is the interaction matrix of **interactions strengths** we want to estimate to calculate community-based stability metrics.
...
\end{equation}
```
Parameters $\mathbf{B}$, $\mathbf{u}$, $\mathbf{C}$, $\mathbf{Q}$, $\mathbf{Z}$, $\mathbf{a}$, $\mathbf{D}$, and $\mathbf{R}$ can be composed of both estimated and fixed values, depending on the assumptions about the system.
...
The motivation for it is to avoid the indiscriminate use, and thus force the user to take into consideration the implication of the chosen assumptions.
