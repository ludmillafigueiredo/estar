...
When calculating the multivariate metrics the user can either provide the required community matrix ($B$) or have it estimated from a variety of predefined assumptions about the modelled system. In this section we detail what are these assumptions and show how they might affect the estimation of ($B$) by applying them to the same study case. None of the parameters detailed here have default values in functions that calculate the metrics above. the motivation for it is to avoid the indiscriminate use, and thus force the user to take into consideration the implication of the assumptions chosen.
