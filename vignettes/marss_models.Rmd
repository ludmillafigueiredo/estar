...
library(cowplot)
library(ggsci)
source("/home/luf74xx/Dokumente/postdoc/eStar_main/eStar/R/marss_buildZ.R")
source("/home/luf74xx/Dokumente/postdoc/eStar_main/eStar/vignettes/custom_aesthetics.R")
```
...
```{r include = FALSE}
load("/home/luf74xx/Dokumente/postdoc/eStar_main/MAR_macroinv_Pesticide/macrofauna4marss_5gps.RData")
```

To illustrate the use of MARSS models inside `eStar`, we use a dataset compiled from an ecotoxicological study about the effects of insecticide use on a macroinvertebrate aquatic community [@vandenbrick_effects_1996; @wijngaarden_effects_1996]. The insecticide chlorpyrifos, which negatively affects consumer, but not producer species [@radchuk_biodiversity_2016]. In response to a pulse disturbance (Glossary in main text), @radchuk_biodiversity_2016 reports decreased and destabilized species diversity, but no effects on gross primary production nor respiration. 
...
```{r fig.height=6, fig.width=8, fig.cap = "Organism count (mean +- sd in grey, log axis) of the five functional groups over the course of ecotoxicological experiment. The vertical green line identifies the aplication of chloryfiros insecticide, and facets separates the different concentrations (mug/L)."}
fgps_5.df %>%
  tidyr::pivot_longer(cols = c(Herbivores, DetHerbivores, Carnivores, Omnivores, Detrivores),
                      names_to = "fgp",
                      values_to = "count") %>%
  dplyr::group_by(week, treat, fgp) %>%
  dplyr::summarize(count.mu = mean(log(count)),
                   count.sd = sd(log(count))) %>%
  ggplot(aes(x = week, y = count.mu, col = fgp, group = fgp)) +
  geom_ribbon(aes(ymin = count.mu - count.sd,
                  ymax = count.mu + count.sd),
              col = "gainsboro", fill = "gainsboro") +
  geom_point(alpha = 0.5) +
  geom_line() +
  geom_vline(aes(xintercept = 0), 
             colour = ggsci::pal_rickandmorty(palette = "schwifty")(6)[6])+
  facet_wrap(~treat, ncol = 1) +
  ggsci::scale_colour_rickandmorty(palette = "schwifty") +
  labs(x = "Week",
       y = "Count (mean +- sd))",
       colour = "Functional\ngroup") +
  theme_estar()
...
```{r eval=FALSE}
## calculate z-scores
fgps_5z_allscen.ldf <- fgps_5.df %>%
    dplyr::filter(week >= 1 , week <= 28) %>%
    ungroup() %>%
    dplyr::mutate_at(vars(Herbivores, DetHerbivores, Carnivores, Omnivores, Detrivores), 
                     ~MARSS::zscore(.)) %>%
    dplyr::mutate(across(c(Herbivores, DetHerbivores, Carnivores, Omnivores, Detrivores),
                         ~dplyr::na_if(., 0))) %>%
    tidyr::pivot_longer(cols = c(Herbivores, DetHerbivores, Carnivores, Omnivores, Detrivores),
                        names_to = "fgp",
                        values_to = "count")

## summarize counts over replicates
fgps_5z_allscen.summldf <- fgps_5z_allscen.ldf %>%
    dplyr::group_by(week, treat, fgp) %>%
    dplyr::summarize(count_mu = mean (count),
                     count_sd = sd(count)) %>%
    dplyr::ungroup()

## convert into time-series matrix
(fgps_5z_allscen.summmxls <- fgps_5z_allscen.summldf %>%
    dplyr::select(week, treat, fgp, count_mu) %>%
    split(.$treat) %>%
    map(~ dplyr::select(., week, fgp, count_mu) %>%
        unique() %>%
        tidyr::pivot_wider(id_cols = fgp,
                           names_from = week, values_from = "count_mu", 
                           names_prefix = "week ") %>%
...
```{r eval=FALSE}
fgps_5z_allscen.marssls <- fgps_5z_allscen.summmxls %>%
    map(~ MARSS(.,
...
```{r}
(fgps_5.Bls <- fgps_5z_allscen.marssls %>%
  map(~eStar::extractB(., 
