...
We will adress is in the following descriptions:
...
Thus, for our [aquatic commmunity](aquatic-data) affected by insecticide concentration ($I$):
...
For our [aquatic commmunity](aquatic-data) affected by insecticide concentration ($I$)
\begin{equation}
 \left[ \begin{array}{c}
    y_{1} \\
    ... \\
...
- Single observation per state process: $n = m$, thus $y_i = x_i$
- No processes error*: $w_t = 0$
- Observational error is th same for all functional groups: $v_t = v$
- Constant covariates*: $C_{t} = C$ and $c_{t} = c$
- U = A = 0
- $X_0 = X_{t=1}$
...
Such model can be formalized as:

\begin{equation}
X = \mathbf{B}.X_{t-1} + C.c + 0 + 0 \\
