...
#' @export
invariability <- function(svdb_i, tdb_i, mode, tf_invar, db_data = NULL,
  ...
  if (mode == "cv") {
    sv_v <- dplyr::pull(dbts_df, svdb_c)

    if (any(is.na(sv_v))) {
      warning("NAs detected among the entries of the state variable")

      if (sum(!is.na(sv_v)) < 10) {
        warning("Less than 10 data points are available for measuring invariability.")
      }
    }

    invar <- 1 / (stats::sd(sv_v, na.rm = na_rm) / mean(sv_v, na.rm = na_rm))

    return(invar)
  } else {
    if (is.null(svbl_i) | is.null(tbl_i)) {
      stop("Missing 'svbl_i' or 'tbl_i' argument(s).")
    }
    if (mode == "lm_res") {
      ...
      invar_df <- dplyr::inner_join(
        ...
        dplyr::mutate(lrr = log(svdb_c / svbl_c))
